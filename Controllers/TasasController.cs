...
namespace WebApplication2.Controllers
{
    public class TasasController : Controller
    {
        ...
        public async Task<ActionResult> Create([Bind(Include = "Lng_IdDotacion,Lng_IdSucursal,Dbl_Monto,Int_IdMoneda,Fec_Registro,Int_Idusuario,Int_EstatusDot")] Tb_Dotaciones tb_Dotaciones)
        {
            if (ModelState.IsValid)
            {
                db.Tb_Dotaciones.Add(tb_Dotaciones);
                await db.SaveChangesAsync();
                return RedirectToAction("Index");
            }

            return View(tb_Dotaciones);
        }
